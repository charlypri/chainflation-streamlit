...
def plot_priceTime(prod_params):
    ...
    fig = px.line(
        prod_prices[(prod_prices["fuente"] == prod_params)],
        x="fecha",
        y="precio",
        color="producto",
    )
...
def plot_prod_infTime():
    sums = (
        sources_infl.groupby(["fecha", "fuente"]).mean(numeric_only=True).reset_index()
    )
...
def plot_prod_comTime(prod_params):
    ...
    fig = px.line(
        prod_prices[(prod_prices["producto"] == prod_params)],
        x="fecha",
        y="precio",
        color="fuente",
    )
...
def filter_df_by_date(df, time_period):
    ...
    if time_period == "Semana":
        ...
        raise ValueError(
            "Invalid time_period argument. Possible values: 'last_week', 'last_month', 'last_year'."
        )
...
if selected == "Sectores":
    ...
    with col2:
        param_prod = st.selectbox("Selecciona la fuente", sources_names)

    with col3:
        periodo = st.selectbox(
            "Selecciona el periodo", ["Semana", "Mes", "Trimestre", "Año"]
        )
    ...
    if param_cesta == "alimentacion":
        st.subheader(f"Compara precios entre supermercados:")
        param_prod = st.selectbox("Selecciona el producto a comparar", product_names)
        fig = plot_prod_comTime(param_prod)
        st.plotly_chart(fig)

    # SECCION DE INFLACION
    st.subheader(f"Inflación por fuente:")
